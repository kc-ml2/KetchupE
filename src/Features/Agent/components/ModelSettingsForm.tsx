...
const ModelSettingsForm = ({ api }: { api: KetchupEAgentAPI }): React.JSX.Element => {
  ...
  useEffect(() => {
    void api.getModelSettings().then((settings) => {
      ...
      if (settings.hasApiKey) void api.listModels().then(setModels).catch(() => setModels([]));
    });
  }, [api]);
  ...
  const save = async (event: React.FormEvent) => {
    ...
    try {
      await api.setModelSettings({ baseURL, modelAlias, apiKey: apiKey || undefined });
      setApiKey("");
      setHasApiKey(hasApiKey || apiKey.length > 0);
      const result = await api.testModelConnection();
      setModels(result.models);
      setMessage(result.ok ? `연결 확인. 사용 모델: ${result.resolvedAlias}` : "저장했지만 모델 목록을 받지 못했습니다. 도메인과 키를 확인하세요.");
    } catch (cause) {
  ...
  return (
    <form onSubmit={save} className="flex flex-col gap-3 text-sm">
      ...
      <label className="flex flex-col gap-1">
        <span className="text-xs text-[#71717A]">모델 (선택, 비우면 서버의 첫 번째 모델)</span>
        {models.length ? (
          <select value={modelAlias} onChange={(event) => setModelAlias(event.target.value)} className={field}>
          ...
          <input value={modelAlias} onChange={(event) => setModelAlias(event.target.value)} placeholder="자동" className={field} />
        )}
      </label>
      ...
      <p className="text-[11px] text-[#71717A]">키는 OS 보안 저장소로 암호화되며 화면, trace, export에 노출되지 않습니다.</p>
    </form>
  );
};
